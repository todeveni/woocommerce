...
class WC_Product_Variation extends WC_Product_Simple {
	...
	*/
	 ...
	 *
	 * If "additive" flag is set, the effective value is equal to sum of the defined values for the variation and the parent product.
	 * Otherwise, if the defined value for this variation is null, the effective value is equal to the defined value for the parent product.
	 * Otherwise, the effective value is equal to the defined value for this variant.
	 *
	 * @return float
	 */
	protected function get_cogs_effective_value_core(): float {
		$own_value = $this->get_cogs_value();
		if ( $this->get_cogs_value_is_additive() ) {
			return $this->get_parent_cogs_effective_value() + $own_value;
		}
		else {
			return is_null($own_value) ? $this->get_parent_cogs_effective_value() : $own_value;
		}
	}
